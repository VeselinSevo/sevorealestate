...
async function getCountries(preselectCountryId = null) {
    try {
        ...
        renderCountriesDdl(countries, preselectCountryId);
    } catch (error) {
        console.error('Error fetching data:', error);
    }
}

async function getCities(countryId, preselectCity = null) {
    try {
        ...
        renderCitiesDdl(cities, preselectCity);
    } catch (error) {
        console.error('Error fetching data:', error);
    }
}
...
function renderCountriesDdl(countries, preselectCountryId = null) {
    var countryDdl = document.getElementById('countryDdl');
    console.log('preselectCountryId')
    countryDdl.innerHTML = ``;
    countries.forEach((country, index) => {
        const selected = country.id === preselectCountryId ? 'selected' : '';
...
function renderCitiesDdl(cities, preselectCityId = null) {
    console.log(preselectCityId)
    ...
    cities.forEach((city, index) => {
        const selected = city.id === preselectCityId ? 'selected' : '';
        cityDdl.innerHTML += `
...
console.log(preselectCountryId)
getCountries(preselectCountryId);
getTypes()
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Preselect country and city if they are provided as data
    if (preselectCountryId) {
        countrySelect.value = preselectCountryId;
        getCities(preselectCountryId); // Fetch cities for the selected country
    }
    if (preselectCityId) {
    ...
    function prepopulateMainImageInput() {

    }

    function prepopulateAdditionalImageInput(fileNames) {
        // const dT = new DataTransfer();
        //
    ...
    async function renderImages() {
        // const container = document.querySelector('#additional-images-preview-container');
        // container.innerHTML = '';

        let imageUrls = await fetchImages(propertyId);

        prepopulateAdditionalImageInput(imageUrls)
